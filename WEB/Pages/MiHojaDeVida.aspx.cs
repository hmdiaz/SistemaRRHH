...
namespace WEB.Pages
{
    public partial class MiHojaDeVida : PageHelper
    {
        ...
        {
            UsuarioObj = new ResponseLogin();
            if (!Page.IsPostBack)
            {
                Seguridad();
                UsuarioObj = (ResponseLogin)Session["SessionSecurity"];
                Session["UsuarioObj"] = UsuarioObj;
            }
            else
            {
                UsuarioObj = (ResponseLogin)Session["UsuarioObj"];
            }
            UsuarioID = UsuarioAuditado;
            bool ConsultarEmpleado = false;
            List<Sistema> sistemas = new List<Sistema>();
            sistemas = UsuarioObj.Systems;
            foreach (Sistema sistema in sistemas) {
                foreach (Pantalla pantalla in sistema.pantallas)
                {
                    if (pantalla.pantalla == "CONSULTAEMPLEADOSRRHH")
                    {
                        ConsultarEmpleado = true;
                    }
                }
            }
            Session["ConsultarEmpleados"] = ConsultarEmpleado;
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            CompararFechaMayor.ValueToCompare = DateTime.Now.Year +"-" + DateTime.Now.Month + "-" +DateTime.Now.Day;
            //CompararFechaRetiroExpLaboral.ValueToCompare = DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day;
            ...
            using (EmpleadoServiceClient svcEmpleado = new EmpleadoServiceClient())
            {
                if (!IsPostBack)
                {
                    ...
                    WEB.EmpleadoService.Empleado empleado = svcEmpleado.ObtenerEmpleadoByUsuarioID(UsuarioID).FirstOrDefault();
        ...
        {
            // Con base en la sesión construyo el nombre del usuario a mostrar en la aplicación
            StringBuilder sbUsuario = new StringBuilder();
            sbUsuario.Append(UsuarioObj.PrimerNombre);
            sbUsuario.Append(" ");
            sbUsuario.Append(UsuarioObj.SegundoNombre);
            sbUsuario.Append(" ");
            sbUsuario.Append(UsuarioObj.PrimerApellido);
            sbUsuario.Append(" ");
            sbUsuario.Append(UsuarioObj.SegundoApellido);

            usuario = sbUsuario.ToString();
            usuarioID = UsuarioObj.UserID;
        ...
        protected void btnActualizarInformacion_Click(object sender, EventArgs e)
        {
            using (EmpleadoServiceClient svcEmpleado = new EmpleadoServiceClient())
            {
                ...
                int n = svcEmpleado.ActualizarInformacionUsuario(UsuarioID, PrimerNombre, SegundoNombre,
        ...
        protected void btnGuardarIntegrante_Click(object sender, EventArgs e)
        {
            ...
            txtOcupacionIntegrante.Text = string.Empty;
            txtNombresIntegrante.Text = string.Empty;
            txtApellidosIntegrante.Text = string.Empty;
            txtFechaNacimientoIntegrante.Text = string.Empty;
        ...
        public void CargarGrids() {
            using (EmpleadoServiceClient svcEmpleado = new EmpleadoServiceClient())
            {
                dgvInformacionFamiliar.DataSource = svcEmpleado.ObtenerInformacionFamiliarEmpleadoByUsuarioID(UsuarioID);
                dgvInformacionFamiliar.DataBind();

                dgvFormacionAcademica.DataSource = svcEmpleado.ObtenerFormacionAcademicaEmpleadoByUsuarioID(UsuarioID);
                dgvFormacionAcademica.DataBind();

                grvExperienciaLaboral.DataSource = svcEmpleado.ObtenerExperienciaLaboralEmpleadoByUsuarioID(UsuarioID);
        ...
        protected void btnGuardarEstudio_Click(object sender, EventArgs e)
        {
            using (EmpleadoServiceClient svcEmpleado = new EmpleadoServiceClient())
            {
                ...
                string Ocupacion = txtOcupacionIntegrante.Text.Trim().ToUpper();
                DateTime FechaInicio = DateTime.Parse(txtFechaInicioEstudio.Text);
                DateTime? FechaFinalizacion = null;

                if (!string.IsNullOrEmpty(txtFechaFinalizacionEstudio.Text))
                {
                    FechaFinalizacion = Convert.ToDateTime(txtFechaFinalizacionEstudio.Text);
                }
                else 
                { 
                ...
                // FechaFinalizacion = txtFechaFinalizacionEstudio.Text == null ? DateTime.Parse(txtFechaFinalizacionEstudio.Text.Trim()) : null;
            ...
            txtFechaInicioEstudio.Text = string.Empty;
            txtFechaFinalizacionEstudio.Text = string.Empty;
            txtInstitucion.Text = string.Empty;
            txtTituloObtenido.Text = string.Empty;
            ddlPaisInfoAcademica.SelectedIndex = 0;
            ddlNivelFormacion.SelectedValue = "";
            ddlMunicipioInfoAcademica.Items.Clear();
            ddlMunicipioInfoAcademica.Items.Insert(0, new ListItem("[Seleccione uno...]", ""));
            ddlDepartamentoInfoAcademica.Items.Clear();
            ddlDepartamentoInfoAcademica.Items.Insert(0, new ListItem("[Seleccione uno...]", ""));
        }
        ...
        protected void btnGuardarExperienciaLaboral_Click(object sender, EventArgs e)
        {
            ...
            ddlPaisExperienciaLaboral.SelectedIndex = 0;
            ddlMunicipioExperienciaLaboral.Items.Clear();
            ddlMunicipioExperienciaLaboral.Items.Insert(0, new ListItem("[Seleccione uno...]", ""));
            ddlDepartamentoExperienciaLaboral.Items.Clear();
            ddlDepartamentoExperienciaLaboral.Items.Insert(0, new ListItem("[Seleccione uno...]", ""));
